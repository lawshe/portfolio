Meteor.fxns = {
	...
	checkMenu: function(){
		if($('.menu-wrap').hasClass('menu-visible')){
	...
	toggleMobileMenu: function() {
		...
		$('.main-menu > li').removeClass('hover_bg');
		$('#fullpage').toggleClass('fullpage-covered');

		if($('.menu-wrap').attr('class') === 'menu-visible'){
	...
	removeMenuClasses: function(){
		//--mobile menu
			...
			$('#fullpage').removeClass('fullpage-covered');
	},
	...
	imagePath: function(project,slug){
		var imgSize = Session.get('imgSize');
		project['title']['imagePath'] = '/img/work/' + slug+ '/' + imgSize + '/' + project['title']['image'];

		$(project['sections']).each(function(i,v){
			if(v['project']){
				...
				$(project).each(function(index,value){
					if(value['images']){
						$(value['images']).each(function(ind,val){
							val['imagePath'] = '/img/work/' + slug+ '/' + imgSize + '/' + val['image'];
						});
					}
				});
			}
